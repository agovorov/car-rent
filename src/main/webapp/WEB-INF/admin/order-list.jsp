...
<a:admin_page title="${pageHeader}" cssitem="/css/order.css">
        ...
        <table class="table table-striped">
            ...
            <c:forEach items="${orders}" var="order" varStatus="s">
                ...
                <tr class="row-${s.count % 2}">
                    ...
                    <td class="actions-href"><a href="/controller?action=order-detail&id=${order.getId()}"><fmt:message
                            key="order.confirm.detail"/></a>

                        <c:if test="${order.getStatus() == ORDER_CONFIRMED}">
                            <a href="/controller?action=order-pay&id=${order.getId()}"><fmt:message key="order.pay.for.it"/></a>
                        </c:if>

                        <c:if test="${order.getStatus() == ORDER_PAYED}">
                            <a href="/controller?action=order-issue&id=${order.getId()}"><fmt:message key="order.give.a.car"/></a>
                        </c:if>

                        <c:if test="${order.getStatus() == ORDER_VEHICLE_RETURNED}">
                            <a href="/controller?action=order-received&id=${order.getId()}"><fmt:message key="order.vehicle.received"/></a>
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
